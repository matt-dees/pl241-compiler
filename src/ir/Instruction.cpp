...
Instruction::Instruction(int Id, vector<Value *> &&Arguments)
...
BasicBlockTerminator::BasicBlockTerminator(int Id, vector<Value *> &&Arguments)
...
void ConditionalBlockTerminator::updateTarget(BasicBlock *OldTarget,
  ...
  *OldTargetIt = NewTarget;
  ...
  OldPred.erase(remove(OldPred.begin(), OldPred.end(), getOwner()),
                OldPred.end());
...
namespace {
vector<Value *> prepareCallArguments(Function *Target,
